...
export function verifyJwtIatOrThrow(options: {
  now?: Date;
  iat: number;
}) {
  ...
  if (nowSeconds - options.iat > MAX_IAT_AGE_SECONDS) {
    throw new Error(
      "iat claim in JWT is too old (must be within 5 minutes)",
    );
  }

  if (options.iat - nowSeconds > CLOCK_SKEW_TOLERANCE_SECONDS) {
    throw new Error(
      "iat claim in JWT is too far in the future",
    );
  }
}