...
import { PushedAuthorizationRequestSigned, PushedAuthorizationResponse, zPushedAuthorizationResponse } from "./z-authorization-request";
...
export async function fetchPushedAuthorizationRequest(
    ...
    try {
        ...
        const parsedParResponse = zPushedAuthorizationResponse.safeParse(parResponseJson);
        ...
        if (error instanceof PushedAuthorizationRequestError || error instanceof Oauth2ParseError) {
            throw error;
        }
        throw new PushedAuthorizationRequestError(
